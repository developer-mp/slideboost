...
async function sendVideoViaWebSocket(ws, file) {
  ...
  try {
    ...
    console.log(
      `Sending file in ${totalChunks} chunks of ${chunkSize} bytes each`
    );
...
async function closeElectronApp(websocket) {
    ...
    try {
      websocket.send(
        JSON.stringify({
          ...
          message:
            "Electron app closing via npm script, server will remain running",
          timestamp: new Date().toISOString(),
        })
      );
    ...
    try {
      ...
      const gracefulClose = await new Promise((resolve) => {
        const timeout = setTimeout(() => {
          console.log("Graceful shutdown timeout, trying npm stop script...");
          resolve(false);
        }, 2000);
      ...
      if (!gracefulClose) {
        console.log(
          "🔧 Using platform-specific npm script to stop Electron..."
        );
      ...
      if (electronProcess && !electronProcess.killed) {
        console.log(
          "🔫 npm scripts failed, using direct spawn kill commands..."
        );
        ...
        await new Promise((resolve) => {
          ...
          killProcess.on("close", (code) => {
            if (code === 0) {
              console.log(
                "💥 Electron process terminated via direct spawn command"
              );
            } else {
  ...
  console.log(
    "✅ Electron app closed via spawn method, server ready for new connections"
  );
}
...
wss.on("connection", (ws, req) => {
        ...
        if (ws.readyState === WebSocket.OPEN) {
          ws.send(
            JSON.stringify({
              type: "closing-app",
              message:
                "Video confirmed received, closing Electron app via npm scripts",
              timestamp: new Date().toISOString(),
            })
          );
        }
    ...
    if (currentWebSocket && currentWebSocket.readyState === WebSocket.OPEN) {
      ...
      sendVideoViaWebSocket(currentWebSocket, videoData)
        .then((result) => {
          if (result.success) {
            console.log(
              "✅ Video sent successfully, waiting for client confirmation..."
            );
          } else {
  ...
  electronProcess.on("close", (code, signal) => {
    console.log(
      `🔚 Electron process exited with code: ${code}, signal: ${signal}`
    );
    electronProcess = null;
  ...
  electronProcess.on("exit", (code, signal) => {
    console.log(
      `🚪 Electron process exit event - code: ${code}, signal: ${signal}`
    );
    electronProcess = null;
  });
...
const statusInterval = setInterval(() => {
  ...
  console.log(
    `📡 Current WebSocket available: ${currentWebSocket ? "Yes" : "No"}`
  );
  console.log(`📤 Upload in progress: ${isUploadingFile ? "Yes" : "No"}`);
  console.log(
    `📱 Electron process running: ${
      electronProcess && !electronProcess.killed
        ? "Yes (PID: " + electronProcess.pid + ")"
        : "No"
    }`
  );
